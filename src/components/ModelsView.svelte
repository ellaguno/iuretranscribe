...
<header class="top">
  ...
  <div class="hactions">
    ...
    <button class="btn sm" onclick={openDir} title={app.sys?.modelsDir}><Icon name="folder" size={15} /> {t("models.openFolder")}</button>
  </div>
</header>
  ...
  {#each app.models as m (m.id)}
    ...
    <div class="card model" class:current>
      <div class="mhead">
        <div class="mtitle">
          <h3>{m.name}</h3>
          {#if m.recommended}<span class="pill accent">{t("models.recommended")}</span>{/if}
          {#if m.bundled}<span class="pill">{t("models.bundled")}</span>{/if}
          {#if current}<span class="pill success"><Icon name="check" size={11} stroke={3} /> {t("models.inUse")}</span>{/if}
        </div>
      ...
      <div class="mfoot">
        ...
        {#if dl}
          ...
          <button class="btn sm danger" onclick={() => cancelDownload(m.id)}><Icon name="x" size={14} /> {t("models.cancel")}</button>
        {:else if m.downloaded}
          {#if !current}
            <button class="btn sm primary" onclick={() => use(m.id)}>{t("models.use")}</button>
          {/if}
          {#if !m.bundled}
            <button class="btn sm ghost danger" title={t("models.delete")} onclick={() => deleteModel(m.id)} disabled={app.running}><Icon name="trash" size={14} /></button>
          {/if}
        {:else}
          <button class="btn sm primary" onclick={() => downloadModel(m.id)}><Icon name="download" size={14} /> {t("models.download")}</button>
        {/if}
      </div>
    </div>
  {/each}
